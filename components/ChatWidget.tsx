...
/**
 * Chat with a character, streamed from /api/chat. Replies are generated by a
 * model that Ollama runs on the server, so the header shows which model spoke
 * and the panel explains precisely what to do when it is not ready yet.
...
/** Mirrors the ChatStatus union returned by GET /api/chat. */
type ChatStatus =
  | { ready: true; model: string; parameterSize: string | null }
  | { ready: false; reason: "unreachable" | "model_missing"; model: string; available: string[]; detail: string };
...
export function ChatWidget({ character }: { character: Character }) {
  ...
  /** null while the readiness probe is in flight. */
  ...
  const composerVisible = !notReady && !capReached;
    ...
    async (history: ChatMessage[]) => {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          // The server knows more than our cached probe — re-ask and show its notice.
          if (res.status === 503 && (body.error === "model_missing" || body.error === "ollama_unreachable")) {
            setStatus({
              ready: false,
              reason: body.error === "model_missing" ? "model_missing" : "unreachable",
              model: "",
              available: [],
              detail: body.message ?? "",
            });
  ...
  const showStarters = hydrated && transcript.length === 0 && !notReady && !streaming;
  ...
  return (
    <section aria-labelledby={headingId} className="card overflow-hidden">
      ...
      <header className="flex items-center gap-3 border-b border-white/10 bg-linear-to-r from-gold-500/10 via-transparent to-transparent px-4 py-3 sm:px-5">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="truncate text-xs uppercase tracking-[0.2em] text-parchment-dim">
            In character{status?.ready ? ` · ${status.model} via Ollama` : ""}
          </p>
        </div>
      ...
      >
            ...
            {status.reason === "model_missing" ? (
              <>
                <p className="mt-1 text-muted">
                  Ollama is running, but the model {character.name} thinks with has not been pulled yet. On the machine
                  running Ollama:
                </p>
                <pre className="mt-2 overflow-x-auto rounded bg-night-900/80 px-3 py-2 text-xs text-gold-200">
                  ollama pull {status.model || "llama3.2:1b"}
                </pre>
                {status.available.length > 0 && (
                  <p className="mt-2 text-xs text-muted">
                    Installed right now: {status.available.join(", ")}. Set{" "}
                    <code className="rounded bg-night-900/80 px-1 py-0.5 text-gold-200">OLLAMA_MODEL</code> to use one of
                    those instead.
                  </p>
                )}
              </>
              ...
              <>
                <p className="mt-1 text-muted">
                  No Ollama server is answering, so nobody can voice {character.name}. Start one locally with{" "}
                  <code className="rounded bg-night-900/80 px-1 py-0.5 text-gold-200">ollama serve</code>, or point{" "}
                  <code className="rounded bg-night-900/80 px-1 py-0.5 text-gold-200">OLLAMA_BASE_URL</code> at the
                  server that runs it.
                </p>
                <p className="mt-2 text-xs text-muted">
                  Everything else on this page keeps working without it.
                </p>
              </>
            )}
          </div>
        ...
        {hydrated && capReached && !notReady && !lastIsUser && (
